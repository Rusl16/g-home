...
<div id="comments"> <?php // див с этим id нужен для якорьных ссылок на комменты ?>
	<h2 class="discussion-comments">Комментарии (<?php echo get_comments_number(); // общие кол-во комментов ?>)</h2>
	<?php if (comments_open()) { // если комментирование включено для данного поста
		...
		$args = array( // опции формы комментирования
			...
			'comment_field' => '<div class="form-group"><span class="grey-icon"></span><textarea class="form-control write-comment" id="comment" name="comment" placeholder="Присоединиться к обсуждению" cols="45" rows="8" required></textarea></div>', // разметка поля для комментирования
