...
<div id="comments"> <?php // див с этим id нужен для якорьных ссылок на комменты ?>
	<h2>Комментариев <span>(<?php echo get_comments_number(); // общие кол-во комментов ?>)</span></h2>
	<?php if (have_comments()) : // если комменты есть ?>
		<?php if (comments_open()) { // если комментирование включено для данного поста
			...
			$fields =  array( // разметка текстовых полей формы
				'author' => '<div class="form-group --frm_n"><label for="author">Имя</label><input class="form-control" id="author" name="author" type="text" value="'.esc_attr($commenter['comment_author']).'" size="30" required></div>', // поле Имя
...
//				'url' => '<div class="form-group"><label for="url">Сайт</label><input class="form-control" id="url" name="url" type="text" value="'.esc_attr($commenter['comment_author_url']).'" size="30"></div>', // поле сайт
			...
			$args = array( // опции формы комментирования
				...
				'comment_field' => '<div class="form-group --frm"><img src="/wp-content/themes/garnet-theme/img/comment.jpg" alt=""><label for="comment"></label><textarea placeholder="Присоединиться к обсуждению" class="form-control" id="comment" name="comment" cols="45" rows="8" required></textarea></div>', // разметка поля для комментирования
