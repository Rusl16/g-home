...
class clean_comments_constructor extends Walker_Comment { // класс, который собирает всю структуру комментов
    ...
    protected function comment( $comment, $depth, $args ) { // разметка каждого комментария, без закрывающего </li>!
		...
		echo '<span class="share"> Поделиться </span>'; //для share
		$args = array(
			'comment_field' => '<div class="form-group"><span class="grey-icon"></span><textarea class="form-control write-comment" id="comment" name="comment" placeholder="Присоединиться к обсуждению" cols="45" rows="8" required></textarea></div>'
		);
		comment_form($args);
        echo '</div>'."\n"; // закрываем див
    }
